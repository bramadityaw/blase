//! Adapted from https://github.com/rust-lang/rust-analyzer/blob/b8458013c217be4fccefc4e4f194026fa04ab4ca/crates/ide-db/src/text_edit.rs
//! Representation of a `TextEdit`.
//!
//! `blase` never mutates text itself and only sends diffs to clients,
//! so `TextEdit` is the ultimate representation of the work done by blase.
...
impl InsertDelete {
    pub fn insert(offset: TextSize, text: String) -> InsertDelete {
        InsertDelete::replace(TextRange::empty(offset), text)
    }
    pub fn delete(range: TextRange) -> InsertDelete {
        InsertDelete::replace(range, String::new())
    }
    pub fn replace(range: TextRange, replace_with: String) -> InsertDelete {
        InsertDelete {
            delete: range,
            insert: replace_with,
        }
...
impl TextEdit {
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, InsertDelete> {
        self.into_iter()
    }
    ...
    pub fn apply(&self, text: &mut String) {
        ...
        for indel in &self.indels {
            ...
            max_total_len = std::cmp::max(max_total_len, total_len);
        }
...
impl IntoIterator for TextEdit {
    ...
    type IntoIter = std::vec::IntoIter<InsertDelete>;
...
impl<'a> IntoIterator for &'a TextEdit {
    type Item = &'a InsertDelete;
    type IntoIter = std::slice::Iter<'a, InsertDelete>;
...
impl TextEditBuilder {
    ...
    pub fn replace(&mut self, range: TextRange, replace_with: String) {
        self.indel(InsertDelete::replace(range, replace_with));
    }
    pub fn delete(&mut self, range: TextRange) {
        self.indel(InsertDelete::delete(range));
    }
    pub fn insert(&mut self, offset: TextSize, text: String) {
        self.indel(InsertDelete::insert(offset, text));
    }
    pub fn finish(self) -> TextEdit {
        let TextEditBuilder { mut indels } = self;
    ...
    pub fn indel(&mut self, indel: InsertDelete) {
...
fn assert_disjoint_or_equal(indels: &mut [InsertDelete]) {
    assert!(check_disjoint_and_sort(indels));
}

fn check_disjoint_and_sort(indels: &mut [InsertDelete]) -> bool {
...
where
    I: std::iter::Iterator<Item = &'a InsertDelete> + Clone,
{
    indels
        .clone()
        .zip(indels.skip(1))
...
fn coalesce_indels(indels: Vec<InsertDelete>) -> Vec<InsertDelete> {
    indels
        .into_iter()
        .coalesce(|mut a, b| {
