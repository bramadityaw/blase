...
use async_lsp::lsp_types::{
    ...
    DidSaveTextDocumentParams, InitializedParams, TextDocumentIdentifier, TextDocumentItem,
};
...
pub fn handle_did_close(
    ...
    if server.documents.remove(url).is_none() {
        tracing::error!(url = url.path(), "orphan DidCloseTextDocument");
    }
    ControlFlow::Continue(())
}
